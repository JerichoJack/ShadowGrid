...
const BACKEND_SATELLITE_IMAGERY_PROVIDER = (process.env.VITE_SATELLITE_IMAGERY_PROVIDER || DOTENV_VARS.get('VITE_SATELLITE_IMAGERY_PROVIDER') || 'auto').toLowerCase();
...
const OPENSKY_MIN_INTERVAL_MS = 22_000; // Throttle OpenSky API requests to at most 1 per 22 seconds (4000/day)
...
async function getFlightsPayload(query = {}) {
  ...
  const bounds = parts ? normalizeBounds(parts) : null;
...
// --- New Aircraft Info Proxy Endpoint ---
...
// --- New endpoint handler for aircraft info proxy ---
...
// --- Typecode to Icon/Scale --- Also found in `flights.js` as SHAPES, TypeDesignatorIcons, TypeDescriptionIcons, CategoryIcons, MILITARY_HEX_PREFIXES, ext. but may need to be expanded or adjusted for the unified schema and new data sources. Consider whether this mapping should be centralized in one place (e.g. `flights.js`) or if it makes sense to have a separate mapping here for the aircraft info endpoint, especially if the unified schema introduces new fields that could be used for icon classification (e.g. categoryDescription, classification).
// Expanded: Full mapping from src/layers/flights.js TypeDesignatorIcons

const shapes = {
  // A320 + B737 family icons: thank you pimlie for providing refined svgs
  'a319': {
    w: 23,
    h: 32,
