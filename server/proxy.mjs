...
// #sym:loadDotEnvVars
// Loads environment variables from a .env file into a Map for configuration.
...
// --- Typecode to Icon/Scale mapping (from PowerShell script) ---
...
async function getMarinePayload(bounds) {
  ...
  async function fetchAisstreamVessels(bounds) {
    ...
    ws.on('open', () => {
      ...
      // Per aisstream.io docs, BoundingBoxes is an array of [SW, NE] points
      // { "APIKey": <key>, "BoundingBoxes": [ [ [minLat, minLon], [maxLat, maxLon] ] ], "FilterMessageTypes": ["PositionReport"] }
