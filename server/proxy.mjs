...
// #sym:isLikelyCommercialCallsign
// Determines if a callsign is likely to be commercial based on known airline prefixes.
  if (!m) return false;
...
// #sym:classifyAircraft
// Classifies an aircraft as 'emergency', 'ground', 'military', 'commercial', or 'other' based on callsign, type, and other evidence.
...
// #sym:loadAircraftDatabaseSync
// Loads aircraft, type, and manufacturer databases from CSV files into memory for enrichment and lookup.
...
// #sym:classifyAircraftServer
// Server-side aircraft classification logic, mirrors client logic for consistency.
...
// #sym:enrichAircraftFromDb
// Enriches an aircraft object with additional info from the loaded databases and infers icon/type/category if missing.
...
// #sym:ensureCacheDir
// Ensures the main cache directory exists on disk.
...
// #sym:ensureTileCacheDir
// Ensures the tile cache directory exists on disk.
...
// #sym:ensureCameraStreamDir
// Ensures the camera stream cache directory exists on disk.
...
// #sym:sleep
// Returns a promise that resolves after a given number of milliseconds (async sleep).
function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

// #sym:clamp
// Restricts a value to a given min/max range.
function clamp(value, min, max) {
  return Math.max(min, Math.min(max, value));
}

// #sym:normalizeIsoDate
// Normalizes a date value to ISO string (YYYY-MM-DD), with fallback to today if invalid.
...
// #sym:canonicalizeBandExpression
// Normalizes a comma-separated band expression for satellite imagery requests.
...
// #sym:getSatelliteCredentialSetupHint
// Returns a setup hint for Sentinel Hub satellite imagery credentials.
...
// #sym:getCopernicusDataspaceSetupHint
// Returns a setup hint for Copernicus Data Space satellite imagery credentials.
...
// #sym:getCollectionBackendPolicy
// Returns backend policy and allowed sources for a given satellite imagery collection.
...
// #sym:validateSatelliteImageryRequest
// Validates a satellite imagery request for supported sources, collections, and bands.
...
// #sym:computeImageryRectangle
// Computes a bounding rectangle for satellite imagery preview based on lat/lon and radius.
...
// #sym:getImageryIntent
// Determines if a satellite imagery request is for false color, and provider preferences.
...
// #sym:getConfiguredSatelliteProvider
// Returns the configured satellite imagery provider from environment variables.
...
// #sym:getNasaGibsLayerCandidates
// Returns candidate NASA GIBS WMS layers for a given collection and bands.
...
// #sym:pickSatelliteSourceOrder
// Determines the order of satellite imagery providers to try for a request.
...
// #sym:buildWmsPreviewUrl
// Constructs a WMS preview URL for satellite imagery.
...
// #sym:verifyPreviewUrl
// Verifies that a preview URL returns a valid response (used for satellite imagery).
...
// #sym:buildNasaGibsPreview
// Builds a NASA GIBS preview for satellite imagery, trying candidate layers.
...
// #sym:buildSentinelHubPreview
// Builds a Sentinel Hub preview for satellite imagery.
...
// #sym:buildCopernicusDataspacePreview
// Builds a Copernicus Data Space preview for satellite imagery.
...
// #sym:resolveSatelliteImageryPreview
// Resolves a satellite imagery preview by trying providers in order of preference.
...
// #sym:ensureFfmpegAvailable
// Checks if ffmpeg is available on the server for camera stream transcoding.
...
// #sym:startCameraStreamCleanupLoop
// Starts a periodic cleanup loop for camera stream sessions and cache directories.
...
// #sym:cameraSessionIdForUrl
// Generates a unique session ID for a camera stream URL.
function cameraSessionIdForUrl(url) {
  return crypto.createHash('sha1').update(url).digest('hex').slice(0, 16);
}

// #sym:rewriteM3u8Playlist
// Rewrites M3U8 playlist URLs to route through the local proxy endpoint.
...
// #sym:ensureRtmpSession
// Ensures an RTMP/RTSP camera stream session is running and returns its session object.
...
// #sym:waitForPlaylist
// Waits for an HLS playlist file to appear and become non-empty (for camera streams).
...
// #sym:handleCameraStreamProxy
// Handles proxying of camera streams (HTTP, RTMP, RTSP) for the /api/cameras/stream endpoint.
...
// #sym:handleCameraHlsSegment
// Handles serving HLS video segments for proxied camera streams.
...
// #sym:handleCameraStreamHealth
// Returns health and status info for camera stream sessions and ffmpeg availability.
...
// #sym:tileCachePaths
// Returns the file paths for a tile cache entry (meta and body).
...
// #sym:readTileCache
// Reads a tile cache entry from disk if it exists and is not stale.
...
// #sym:writeTileCache
// Writes a tile cache entry to disk (meta and body files).
...
// #sym:sendBinaryResponse
// Sends a binary HTTP response with given status, headers, and body.
function sendBinaryResponse(res, status, headers, body) {
  res.writeHead(status, headers);
  res.end(body);
}

// #sym:buildGoogleTileUrl
// Constructs a Google Maps tile URL with API key for proxying.
...
// #sym:buildMapTilerTileUrl
// Constructs a MapTiler tile URL with API key for proxying.
...
// #sym:fetchAndCacheTile
// Fetches a map tile from upstream and caches it on disk, with stale fallback.
...
// #sym:handleTileProxy
// Handles proxying of map tile requests for Google/MapTiler endpoints.
...
// #sym:scheduleCacheWrite
// Schedules a write of the in-memory snapshot caches to disk.
...
// #sym:loadSnapshotCacheFromDisk
// Loads snapshot caches from disk at server startup.
...
// #sym:quantize
// Quantizes a value to the nearest grid step (used for bounding box keys).
function quantize(value, step = SNAPSHOT_BOUNDS_GRID_DEG) {
  return Math.round(value / step) * step;
}

// #sym:normalizeBounds
// Normalizes a bounding box array to valid min/max lat/lon values.
...
// #sym:boundsCacheKey
// Generates a cache key string for a bounding box.
...
// #sym:ensureCameraManifest
// Ensures the camera manifest (tile index) is loaded from disk and cached.
...
// #sym:readCameraTile
// Reads a camera tile file from disk and caches its camera list.
...
// #sym:getCameraSnapshot
// Returns a snapshot of cameras within the given bounds, using tile-based caching.
...
// #sym:generateHubGrid
// Generates a grid of hub points for aircraft data tiling (used for hub fetches).
...
// #sym:hubKey
// Returns a unique string key for a hub object.
function hubKey(hub) { return `${hub.lat},${hub.lon}`; }

// #sym:boundsToQueryCenter
// Computes the center and radius for a bounding box, for hub queries.
...
// #sym:fetchReadsbProvider
// Fetches aircraft data from a readsb-compatible provider (e.g., airplanes.live, adsb.lol).
...
// #sym:gcDistanceMeters
// Calculates the great-circle distance in meters between two lat/lon points.
...
// #sym:trafficFlowFromSpeed
// Maps traffic speed labels to density/speed/priority for traffic overlays.
...
// #sym:osmRoadClass
// Maps OSM road tags to traffic density/speed/priority profiles.
...
// #sym:resolveTrafficBackendMode
// Determines which traffic backend to use (Google or OSM) based on config.
...
// #sym:buildOsmTrafficRoads
// Builds a list of traffic roads from OSM Overpass API for the given bounding box.
...
// #sym:getOpenSkyTokenServer
// Retrieves and caches an OpenSky API token for authenticated requests.
...
// #sym:fetchN2yoSnapshot
// Fetches a snapshot of satellites above sample points using the N2YO API.
...
// #sym:googleRoutePairsForBounds
// Computes route pairs for Google traffic overlays within a bounding box.
...
// #sym:buildGoogleTrafficRoads
// Builds traffic road overlays using Google Routes API for the given bounding box.
...
// #sym:parseTLEText
// Parses TLE (two-line element) text into satellite objects for propagation.
...
// #sym:classifySatelliteMilitaryStatus
// Determines if a satellite is likely military based on its name.
...
// #sym:classifySatelliteApplication
// Classifies a satellite's application (astronomical, weather, navigation, etc.) by name.
...
// #sym:classifySatelliteCrewedStatus
// Determines if a satellite is crewed or uncrewed by name.
...
// #sym:classifySatelliteOrbitType
// Classifies a satellite's orbit type (LEO, GEO, etc.) from TLE line 2.
...
// #sym:deriveSatelliteMeta
// Derives metadata for a satellite from its name and TLE.
...
// #sym:categoryForSatelliteMeta
// Maps satellite metadata to a category (debris, rocket, military, etc.).
...
// #sym:ensureSatelliteCatalog
// Ensures the satellite catalog is loaded from the configured provider (CelesTrak, Space-Track, N2YO).
...
// #sym:satelliteSnapshot
// Propagates satellite positions for a snapshot, optionally filtering by category.
...
// #sym:getSatellitesSnapshotPayload
// Returns a snapshot payload of satellites, using cache if available.
...
// #sym:getTrafficPayload
// Returns a traffic overlay payload for the given bounds, using Google or OSM as backend.
...
// #sym:overpassMarineQuery
// Builds an Overpass QL query for marine vessel data in a bounding box.
...
// #sym:vesselTypeFromTags
// Infers vessel type from OSM/Overpass tags.
...
// #sym:buildTrackPoints
// Builds a simulated track (past positions) for a vessel based on heading and speed.
...
// #sym:parseOverpassVessels
// Parses vessel data from Overpass API response.
...
// #sym:parseAisNumber
// Parses a numeric value from AIS vessel data, handling null/empty.
...
// #sym:parseAisVessels
// Parses vessel data from AIS API response.
...
// #sym:buildAisProxyUrl
// Constructs the AIS proxy URL for marine vessel queries.
...
// #sym:fetchAisVessels
// Fetches vessel data from the configured AIS proxy endpoint.
...
// #sym:getMarinePayload
// Returns a marine vessel snapshot payload for the given bounds, using AIS or Overpass as backend.
...
// #sym:fetchText
// Fetches and returns text from a URL with timeout.
...
// #sym:toIsoString
// Converts a value to ISO string, handling numbers as epoch seconds.
...
// #sym:db
// In-memory aircraft database (Map of ICAO hex to aircraft object).
const db = new Map();

// #sym:upsert
// Inserts or updates aircraft in the in-memory database, enriching with DB/type info.
...
// #sym:pruneStale
// Removes stale aircraft entries from the in-memory database.
function pruneStale() {
...
// #sym:hubsForBounds
// Returns a list of hub points that overlap a bounding box, with padding.
...
// #sym:fetchHub
// Fetches aircraft data for a single hub, with per-hub TTL cache.
...
// #sym:getFlightsPayload
// Returns a payload of aircraft in the current viewport, using the configured backend provider or hub fallback.
...
// #sym:handleFlights
// Handles the /api/flights endpoint, returning aircraft payload for the given query.
...
// #sym:getAircraftDebugLogPath
// Returns the path to the aircraft debug log file, ensuring the logs directory exists.
...
// #sym:server
// Main HTTP server for ShadowGrid proxy endpoints and API routes.
